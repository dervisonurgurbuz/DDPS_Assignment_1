...
if [[ $# -lt 1 ]] ; then
	echo ""
	echo "usage: source deploy_hadoop.sh [nodes]"
	echo "for example: source deploy_hadoop.sh node105,node106,node107"
	echo ""
	exit 1
fi
...
cd /var/scratch/$USER/hadoop/etc/hadoop # && cp spark-env.sh.template spark-env.sh && cp workers.template workers
sleep 3
